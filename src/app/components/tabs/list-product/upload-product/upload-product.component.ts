...
export class UploadProductComponent implements OnInit {
  ...
  invalidRate(rate: number) {
    const newNum = new FormControl(rate, [Validators.pattern('^[0-9]+$')]);
        ...
        for (let p = 0; p < data.length; p++) {
          ...
          if (p > 11 && data[p][0] == p - 11) {
            ...
            if (variant.type && variant.name) {
              if (
                !productData.includes(
                  (m: any) => storeProduct.productName == m.productName
                )
              ) {
                ...
                // put varients in productData
                let c = productData.findIndex(
                  (z) => z.productName == storeProduct.productName
                );
                if (c >= 0) {
            ...
            if (
              ...
              !storeProduct.price ||
              this.invalidRate(storeProduct.price) ||
              !storeProduct.cost ||
              this.invalidRate(storeProduct.cost) ||
                  ...
                  if (
                    ...
                    if (
                      ...
                      if (storeProduct.price < storeProduct.cost) {
                        console.log(
                          'Selling price must be greater or equal to Market price.'
                        );
                      } else {
          ...
          if (!storeINFO) {
            ...
            if (uid !== storeINFO.by) {
              ...
              const storeX = {
                ...
                sid: storeINFO.id,
                cat: storeINFO.cat,
                proCat: storeINFO.proCat,
                subCat: storeINFO.subCat,
  ...
  export(
    ...
    const sample: AOA = [
      [null, 'uid', uid, 'Contact', phone, email],
      [null, 'sid', sid, 'Store Category', cat],
      [
        null,
        null,
        null,
