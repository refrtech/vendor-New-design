...
Refr X ${this.data.name}`,
      subject:
        'Hi, Thanks for being a part of ' +
        this.data.name +
        (this.data.name[this.data.name.length - 1] !== 's' ? "'s" : '') +
        ' journey 😊',
      url: this.link,
    };
    ...
    if (type == 'shareWebShareApi') {
      this.shareWebShareApi(data);
    }
  }
  ...
  shareTwitter(data: { message: string; subject: string; url: string }) {
    const textBODY = `${data.subject}\n${data.message}\n${data.url}`;
    const dataX = { message: textBODY, image: '', url: data.url };
    const tw = `https://twitter.com/intent/tweet?via=${'refrclub'}&text=${
      dataX.message
    }`;
    ...
    */ /*, dataX.url*/
    ...
    if (!this.resource.appMode) {
      ...
      this.socialSharing
        ...
        .then((r) => {
          this.socialSharing
            .shareViaWhatsApp(dataX.message, dataX.image)
            .then((res) => {
  ...
  shareSMS(
    via: string,
    data: { message: string; subject: string; url: string }
  ) {
    const textBODY = `${data.subject}\n${data.message}\n${data.url}`;
  ...
  async shareOther(data: { message: string; subject: string; url: string }) {
    ...
    this.socialSharing
      .share(dataX.message, dataX.subject, dataX.file, dataX.url)
      .then(() => {
      })
      .catch((err) => {
        this.resource.startSnackBar('Error: ' + err);
